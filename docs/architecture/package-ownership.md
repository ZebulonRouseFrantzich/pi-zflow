...
| **Suite help / discoverability**                                    | `pi-zflow` (umbrella)                          | Child packages own help metadata (`ZFLOW_HELP_TOPICS`). `pi-zflow-core` provides shared types and renderers. Umbrella aggregates all topics and registers `/zflow-help` with sub-arguments (commands, flow, profiles, plan, review, change, agents, doctor). Startup hint is a short one-time notification. | No other package may register `/zflow-help` or a generic `/help` command                            |
...
Every child package with user-facing commands **must** provide help topic metadata:

1. Create a `src/help.ts` module that exports a `ZFLOW_HELP_TOPICS` array containing typed `ZflowHelpTopic` objects (imported from `pi-zflow-core`).
2. Re-export `ZFLOW_HELP_TOPICS` from the package's main `src/index.ts` so the umbrella extension can discover it.
3. Include:
   - `packageName`, `id`, `title`, `summary` (required)
   - `flowOrder` (recommended — determines position in the recommended workflow)
   - `flowGuidance` (recommended — one-line workflow placement description)
   - `commands` with namespaced `zflow-*` or `zflow_*` names
   - `relatedTopics` (optional — cross-reference IDs)
4. Add a `test/help.test.ts` file validating that topics are exported, namespaced, and structurally valid.

The umbrella `pi-zflow` package aggregates all `ZFLOW_HELP_TOPICS` arrays and renders them via the `/zflow-help` command. No child package may register its own `/zflow-help` or generic `/help` command.

The recommended workflow order (as presented by `/zflow-help flow`) is:

1. **Profile Management** (`pi-zflow-profiles`) — configure active profile and lanes
2. **Read-Only Planning Mode** (`pi-zflow-plan-mode`) — plan before implementing
3. **Agent Setup** (`pi-zflow-agents`) — ensure custom agents are installed
4. **Change Workflows** (`pi-zflow-change-workflows`) — prepare → implement → audit → fix → clean
5. **Code Review** (`pi-zflow-review`) — review changes before merging

Other topics (artifacts, RuneContext, compaction, dispatch) are supplementary and do not have fixed positions in the workflow sequence.
...
```text
/zflow-help                        Suite-level help and discoverability
/zflow-help <topic>                Topic-specific help
...
/zflow-change-audit <change-id>
/zflow-change-fix <change-id>
/zflow-review-code
/zflow-review-pr <url>
/zflow-clean
/zflow-setup-agents
