...
async function runBootstrapChecks(opts: {
  ...
  if (!rtk.available) {
    warnings.push("rtk not found: output compaction will not be available. Install with: npm install -g rtk")
  }
...
async function beforeProfileActivation() {
  ...
  if (report.modelRegistry.unavailableLanes?.length) {
    logger.warn("Some profile lanes cannot resolve:", report.modelRegistry.unavailableLanes)
  }
}
```
