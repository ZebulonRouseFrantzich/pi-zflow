  ...
  if (zflowDispatch && typeof zflowDispatch.runAgent === "function") {
    try {
      ...
      const task = [
        ...
        "  Write commands so they work from the correct owning repo/package root.",
        "  If a group touches a nested repo or nested package with its own `package.json`,",
        "  use an explicit `cd <dir> && ...` prefix when needed instead of assuming the current cwd.",
