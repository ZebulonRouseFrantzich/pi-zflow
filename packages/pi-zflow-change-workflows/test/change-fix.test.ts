...
describe("runDirectFixWorkflow follow-up hardening", { concurrency: false }, () => {
    ...
    try {
      ...
      const result = await runDirectFixWorkflow({
        ...
        fixResult: {
          ...
          planVersion: "v1",
          lifecycleState: "review-failed",
          fixOrchestratorConfig: { ...resolveFixOrchestratorConfig(), maxAttemptsPerFinding: 3 },
        },
      })
  ...
  it("accepts checker alternative_satisfied and superseded verdicts as resolved", async () => {
    const { runDirectFixWorkflow, resolveFixOrchestratorConfig } = await import(
      "../extensions/zflow-change-workflows/orchestration.js"
    )
    const tmpDir = await mkdtemp(join(tmpdir(), "zflow-test-nonliteral-status-"))
    try {
      let calls = 0
      const dispatchService = {
        name: "test-dispatch",
        async runAgent(input: Record<string, unknown>) {
          ...
          if (task.includes("Fix Satisfaction Check")) {
            return {
              ok: true,
              rawOutput: '```json\n{"zflowFixResult":{"status":"alternative_satisfied","findings":[{"findingId":"finding-1","status":"alternative_satisfied","evidence":["src/file.ts:10 — obsolete cleanup path was removed and equivalent collision observability exists"],"changedFiles":[],"validation":["read source"],"reason":"The literal suggested event is superseded by an equivalent observable design."}]}}\n```',
              outputPath: String(input.output),
            }
          }
          return {
            ok: true,
            rawOutput: '```json\n{"zflowFixResult":{"status":"fixed","findings":[{"findingId":"finding-1","status":"fixed","evidence":["src/file.ts:10"],"changedFiles":["src/file.ts"],"validation":["test"],"reason":"fixed"}]}}\n```',
            outputPath: String(input.output),
          }
        },
        async runParallel() { return { ok: false, results: [] } },
      ...
      assert.equal(result.fixed.length, 1)
      assert.equal(result.fixed[0].status, "already-satisfied")
      assert.match(result.fixed[0].reason ?? "", /alternative_satisfied/)
      assert.equal(result.unresolved.length, 0)
    } finally {
      await rm(tmpDir, { recursive: true, force: true })
    }
