...
export async function loadFragment(name) {
    ...
    try {
        ...
        if (nodeErr.code === "ENOENT") {
            throw new Error(`Prompt fragment "${name}" not found at "${filePath}". ` +
 ...
 */
export async function buildReminderInjection(reminders) {
    const parts = [];
 ...
 */
export async function buildModeInjection(mode) {
    try {
 ...
 */
export async function fragmentExists(name) {
    try {
        const filePath = resolveFragmentPath(name);
