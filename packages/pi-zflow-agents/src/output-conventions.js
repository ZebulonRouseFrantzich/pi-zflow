 ...
 * or `write` tools.
 */
export const REPORT_AGENTS = new Set([
    "builtin:scout",
 ...
 */
export const IMPLEMENTATION_AGENTS = new Set([
 ...
 */
export const HYBRID_AGENTS = new Set([
 ...
 */
export const OUTPUT_CONVENTIONS = {
    // Report-style agents
    "builtin:scout": {
        agent: "builtin:scout",
        role: "report",
 ...
 */
export const ALL_AGENTS = new Set([
    ...REPORT_AGENTS,
...
export function getAgentRole(agentName) {
    if (REPORT_AGENTS.has(agentName))
        return "report";
    if (IMPLEMENTATION_AGENTS.has(agentName))
        return "implementation";
    if (HYBRID_AGENTS.has(agentName))
        return "hybrid";
 ...
 * @throws If the agent name is not recognised.
 */
export function getOutputConvention(agentName) {
 ...
 */
export function getReportAgents() {
 ...
 */
export function getImplementationAgents() {
 ...
 */
export function getHybridAgents() {
 ...
 */
export function getOutputInstructions(agentName) {
    ...
    switch (role) {
        case "report": {
            parts.push("", "You are a report-style agent. Your output is a structured markdown report.", "", "### Rules", "", "- **Do not use `edit`, `write`, or mutation-capable tools.**", "  Your tools are limited to read-only operations.", "- **Return structured markdown** with clear headings, severity levels,", "  and actionable detail.", "- **Do not write files.** The orchestrator persists your output into", "  runtime-state directories.", "- **Your report is captured and persisted automatically.** Focus on", "  producing thorough, structured findings.", "", `Expected format: ${convention.outputFormat}`, `Your role: ${convention.description}`);
            break;
        }
        case "implementation": {
            parts.push("", "You are an implementation agent. Your primary output is the changed", "filesystem state in your assigned worktree.", "", "### Rules", "", "- **Use `edit` and `write` tools** to modify files as specified in", "  your assigned plan group.", "- **Run scoped verification** after implementing.", "- **Report a summary** of what was done, what passed/failed, and any", "  observations or residual risks.", "- **Your file changes are the output.** The orchestrator does not", "  need to persist an additional report from you beyond the summary.", "", `Your role: ${convention.description}`);
            break;
        }
        case "hybrid": {
            parts.push("", "You are a hybrid agent with a restricted write channel.", "", "### Rules", "", "- **Use your restricted write tool to produce artifacts.**", "  Do not use `edit` or `write` for unrestricted mutations.", "- **The orchestrator persists your artifacts.**", "- **Return a summary** of what was produced and any observations.", "", `Expected format: ${convention.outputFormat}`, `Your role: ${convention.description}`);
            break;
        }
    }
    return parts.join("\n");
 ...
 */
export function getOrchestratorOutputInstructions() {
    return [
