 ...
 */
function listSourceChainFiles(packageRoot) {
 ...
 */
function agentFileNameToRuntimeName(filename) {
 ...
 * @returns Result with counts of installed/skipped/errored files.
 */
export function installChains(options = {}) {
    const packageRoot = resolvePackageRoot(options.customPackageRoot);
    const chainsDir = getSourceChainsDir(packageRoot);
 ...
 */
export function installAll(options = {}) {
 ...
 */
export function getInstalledAgents(options) {
 ...
 */
export function getInstalledChains(options) {
 ...
 */
export function getChainPath(chainName) {
 ...
 */
export function verifyDiscovery(options = {}) {
    const packageRoot = resolvePackageRoot(options.customPackageRoot);
    const result = {
        success: true,
        validAgents: [],
    ...
    for (const filename of sourceAgentFiles) {
        ...
        if (installed) {
            ...
            if (collision.collides) {
                ...
                result.messages.push(`Warning: Agent "${runtimeName}" may collide with builtin "${collision.builtinName}". ` +
    ...
    // ── Summary ─────────────────────────────────────────────────────
    if (result.success) {
        result.messages.push(`Discovery verification passed: ${result.validAgents.length} agents, ${result.validChains.length} chains.`);
    }
    else {
        result.messages.push(`Discovery verification FAILED: ${result.invalidAgents.length} invalid agents, ${result.invalidChains.length} invalid chains.`);
    }
    if (result.collisions.length > 0) {
        result.messages.push(`Name collisions detected: ${result.collisions.length}. Review the collision list and verify correct behavior.`);
    }
    return result;
}
/**
